...
import { getConjugationHint } from '../../lib/conjugationHints';
...
export function QuizCard({ question, showAnswer, lastCorrect, userAnswer, hint, onDifficulty }: Props) {
  ...
  const conjugationHint = getConjugationHint(verb.group, mode);
  const isWrong = showAnswer && !lastCorrect;
  ...
  return (
    ...
    >
      {/* Inner: shake on wrong answer */}
      <motion.div
        animate={isWrong ? { x: [0, -9, 9, -6, 6, -3, 3, 0] } : { x: 0 }}
        ...
        <div className="p-6 space-y-4">
          ...
          {showAnswer && (
            ...
            >
              ...
              {/* 💡 Why? — conjugation rule hint (only on wrong answers) */}
              {!lastCorrect && conjugationHint && (
                <div className="border-t border-[var(--color-border)] pt-2 mt-1">
                  <button
                    onClick={() => setShowWhy(w => !w)}
                    className="flex items-center gap-1.5 text-[12px] font-semibold text-amber-600 dark:text-amber-400 hover:opacity-80 transition-opacity"
                  >
                    <span>💡</span>
                    <span>{showWhy ? 'Hide rule' : 'Why? Show conjugation rule'}</span>
                    <span className="text-[10px] opacity-60">{showWhy ? '▲' : '▼'}</span>
                  </button>
                  {showWhy && (
                    <motion.div
                    ...
                    >
                      <pre className="text-[12px] text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 rounded-[10px] p-3 font-sans whitespace-pre-wrap leading-relaxed">
                        {conjugationHint}
                      </pre>
                    </motion.div>
                  )}
